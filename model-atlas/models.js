...
"head_dim": 128,
"attention": "GQA",
"attention_detail": "GQA on all 61 layers: 48 Q heads / 8 KV heads, head_dim 128; partial RoPE — only 64 of the 128 head dims are rotary (interleaved split), θ 1e7; no QK-norm, no attention output gate, no sliding window.",
"n_experts": 192,
"active_experts": 8,
"shared_experts": 1,
"vocab_size": 250624,
...
"tie_embeddings": false,
"vision": null,
"notes": "IFM's open-training flagship (LLM360 lineage — xllm training code, Eval360-V2): 375B stored / 23B active sparse MoE, 61 layers with the first 3 dense (d_ff 16,384) and 58 MoE layers (192 routed experts top-8 + 1 shared, expert d_ff 1,792; sigmoid router with gate bias, top-k normalized, scale 2.5). 512K native context. GQA with partial RoPE on 64 of 128 head dims. Apache-2.0 with pretraining + midtraining data, intermediate checkpoints and training code released/planned. ModelScope release is FP8: only routed experts quantized (weights static 128×128 blocks, activations dynamic); attention, shared experts, routers, first-3 dense layers and lm_head stay BF16. On-disk logical param count 379.2B (safetensors index metadata).",
"sources": [
"https://ifm.ai/blog/k2/",
"https://huggingface.co/IFM/K2-Horizon-375B-A23B",
"https://www.modelscope.cn/models/IFM/K2-Horizon-375B-A23B-FP8",
"https://www.modelscope.cn/models/IFM/K2-Horizon-375B-A23B-FP8/resolve/master/config.json"
],
"confidence": "verified",
"dense_first_layers": 3
},
...
"head_dim": 128,
"attention": "hybrid",
"attention_detail": "45 MoVA layers (3–47): q/k/o projections plus a sigmoid value-router (2560→64, top-4, scale 2.5) feeding 64 value experts (each 2560→1024) whose outputs mix the V states, then a per-head softplus output gate (gate_proj, β=ln 2) applied to the attention output; full RoPE (θ 1e7), grouped RMSNorm (2 groups). Layers 0–2 use plain GQA with a single v_proj and no value routing.",
"n_experts": 100,
"active_experts": 8,
"shared_experts": 1,
"vocab_size": 250624,
...
"tie_embeddings": false,
"vision": null,
"notes": "The architecturally novel member of the K2-Horizon family: Mixture-of-Values attention (MoVA) replaces the V projection on the 45 MoE layers with 64 routed value experts (top-4, sigmoid scoring, scale 2.5) plus a per-head softplus output gate — attention-level sparse routing stacked on a 100-expert FFN MoE (top-8 + 1 shared, expert d_ff 768). 36B stored / 4B active, 48 layers, first 3 dense (d_ff 6,144), 512K native context, grouped RMSNorm (2 groups). Apache-2.0; the FP8 release quantizes only the routed FFN experts of layers 3–47 — MoVA value experts, attention and shared experts stay BF16. Frontier-class results at 4B active, beating dense ~30B models.",
"sources": [
"https://ifm.ai/blog/k2/",
"https://huggingface.co/IFM/K2-Horizon-MoVA-36B-A4B",
"https://www.modelscope.cn/models/IFM/K2-Horizon-MoVA-36B-A4B-FP8",
"https://www.modelscope.cn/models/IFM/K2-Horizon-MoVA-36B-A4B-FP8/resolve/master/config.json"
],
"confidence": "verified",
"dense_first_layers": 3,
"attention_split": {
...
"n": 3,
"type": "GQA",
"sub": "v_proj · softplus gate"
},
{
"name": "MoVA sparse",
"n": 45,
"type": "sparse",
"sub": "64 v-experts · top-4"
}
],
"pattern": "gggmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm",
"pattern_map": {
"g": "GQA",
"m": "sparse"
}
...
"head_dim": 128,
"attention": "GQA",
"attention_detail": "Bidirectional (non-causal, is_causal=False) GQA on all 32 layers — a masked diffusion LM, not autoregressive: 32 Q heads / 4 KV heads, head_dim 128, fused QKV projection, per-head RMSNorm QK-norm, partial RoPE (64 of 128 head dims, θ 3e6). Block-diffusion decoding with block_length 32.",
"n_experts": 256,
"active_experts": 8,
"shared_experts": 1,
"vocab_size": 157184,
...
"tie_embeddings": false,
"vision": null,
"notes": "World's first large-scale agentic diffusion language model (Ant Group InclusionAI): a 100B MoE masked-diffusion LM that generates text in parallel blocks with bidirectional attention instead of autoregressive decoding. Levenshtein Editing adds DELETE/INSERT control tokens (ids 156930/156931) so the model can revise its own output structure during generation; trained with L-EBPO (ELBO-based block-level policy optimization) for multi-turn tool use. Block Routing bounds MoE expert activation per 32-token diffusion block (sigmoid group scoring, 8 groups / top-4 groups, expert capacity 48, scale 2.5). First layer dense (d_ff 9216); 31 MoE layers with 256 routed experts top-8 + 1 shared (expert d_ff 1024). Reported 1.64x BF16 decode throughput vs autoregressive Ling-2.6-flash (+18.6% more under FP8). Active params are NOT officially disclosed — the derived top-8+1 routing estimate is ~4.9B; confidence is partial for that reason only (all other fields are config-verified). On-disk 102.9B params (bf16 safetensors index).",
"sources": [
"https://github.com/inclusionAI/LLaDA2.X/blob/main/LLaDA2_2_tech_report.pdf",
"https://huggingface.co/inclusionAI/LLaDA2.2-flash",
"https://huggingface.co/inclusionAI/LLaDA2.2-flash/raw/main/README.md",
"https://huggingface.co/inclusionAI/LLaDA2.2-flash/raw/main/config.json"
],
"confidence": "partial",
"dense_first_layers": 1
},
...
"head_dim": 128,
"attention": "GQA",
"attention_detail": "Bidirectional (non-causal, is_causal=False) GQA on all 20 layers — a masked diffusion LM, not autoregressive: 16 Q heads / 4 KV heads, head_dim 128, fused QKV projection, per-head RMSNorm QK-norm, partial RoPE (64 of 128 head dims, θ 3e6). Block-diffusion decoding with block_length 32.",
"n_experts": 256,
"active_experts": 8,
"shared_experts": 1,
"vocab_size": 157184,
...
"tie_embeddings": false,
"vision": null,
"notes": "Lightweight variant of LLaDA2.2 (released 2026-09-05): a 16B-total / 1.4B-active MoE masked-diffusion language model with bidirectional attention, Levenshtein Editing (DELETE/INSERT control tokens, ids 156930/156931) and L-EBPO agentic RL — long-context tool calling and self-correction at on-device scale. Block Routing restricts MoE expert activation per 32-token diffusion block (sigmoid group scoring, 8 groups / top-4 groups, expert capacity 48, scale 2.5). First layer dense (d_ff 5120); 19 MoE layers with 256 routed experts top-8 + 1 shared (expert d_ff 512). 128K context. LongBench v2 34.99 vs 12.13 for LLaDA2.1-mini. Computed total 16.26B matches the official 16B claim (ratio 0.98).",
"sources": [
"https://github.com/inclusionAI/LLaDA2.X/blob/main/LLaDA2_2_tech_report.pdf",
"https://huggingface.co/inclusionAI/LLaDA2.2-mini",
"https://huggingface.co/inclusionAI/LLaDA2.2-mini/raw/main/README.md",
"https://huggingface.co/inclusionAI/LLaDA2.2-mini/raw/main/config.json"
],
"confidence": "verified",
"dense_first_layers": 1
},
...
"head_dim": 128,
"attention": "GQA",
"attention_detail": "Full attention on all 36 layers, GQA 32 query / 8 KV heads, head_dim 128, RoPE theta 1e6, 40,960-token context. Streaming full-duplex operation: each 1-second input chunk goes through streaming_prefill + streaming_generate on a shared causal timeline that interleaves user audio/video/text, model text/speech output, and <delegate> events; a per-chunk listen/speak decision lets the model perceive continuously while speaking and distinguish backchannels, interruptions, and redirections.",
"n_experts": null,
"active_experts": null,
"shared_experts": null,
"vocab_size": 151752,
...
"tie_embeddings": false,
"vision": {
"encoder": "SigLIP2 (27L/1152d, patch 14)",
"encoder_params_B": 0.4,
"fusion": "adapter",
"notes": "SigLIP2 vision tower (27 layers, 1152-dim, 16 heads, patch 14, image 980) with a perceiver-style resampler (query_num 64) projecting into the 4096-dim decoder; audio enters via a Whisper-Medium encoder (24 layers, 1024-dim, 16 heads, 80 mel bins) with pool step 5 and an audio projection layer. Speech output uses discrete S3 tokens (6,562 audio tokens) decoded by a bundled 20-layer Token2wav streaming flow-matching model (768-dim, 12 heads, 100 mel bins)."
},
"notes": "Ant Group's (Venus Team + Tsinghua) full-duplex realtime interaction system, arXiv 2609.13814: two separately trained 9B checkpoints on a MiniCPM-o 4.5 / Omni-Flow backbone with a Qwen3-8B language model (36L/4096d, verified against config.json). The Omni checkpoint takes streaming video+audio+text and emits text plus native speech; an audio-only sibling shares the backbone. In-stream <delegate> requests run asynchronously in the external Realtime-Venus-Harness (dual-loop runtime) so tool execution never blocks the conversation, and a training-free long-video memory archives and retrieves audio-visual evidence. Full-Duplex-Bench v1.5: responds to 75% of interruptions, continuation 97/88/86% under backchannel/other-directed/background speech, exceeding Gemini 3.1 Live and GPT-4o; StreamingBench 70.2, OVO-Bench 64.7, MMAU 78.0 (Audio). BF16 release, safetensors index 18.74 GB ≈ 9.37B params (README official 9B). Apache-2.0.",
"sources": [
"https://arxiv.org/abs/2609.13814",
"https://huggingface.co/inclusionAI/Realtime-Venus",
"https://huggingface.co/inclusionAI/Realtime-Venus/raw/main/README.md",
"https://huggingface.co/inclusionAI/Realtime-Venus/raw/main/Realtime-Venus-Omni/config.json"
],
"confidence": "verified"
},
{
...
"head_dim": 128,
"attention": "MHA",
"attention_detail": "30 single-stream DiT blocks over one joint sequence of [MLLM caption features ; noise latents]. Per-head QK-RMSNorm; 3-axis RoPE (axes 32 text / 48 / 48 spatial, theta 256) — caption tokens advance the text axis (length 20480), image tokens are indexed by their h/w coordinates on the two 48-dim spatial axes. adaLN-Zero-style modulation: one shared 256-dim embed (timestep + conditioning) sliced per block into scale+gate pairs (scale = 1+Δ, tanh gates); residual updates gated. Two extra refiner blocks appended at the end (a modulated noise-refiner + an unmodulated context-refiner).",
"n_experts": null,
"active_experts": null,
"shared_experts": null,
"vocab_size": null,
...
"tie_embeddings": false,
"vision": null,
"notes": "Ant Group InclusionAI's graphic-design specialist: a 6B text-to-image model for UI, infographics, posters and other text-rich visuals, with native RGBA output (transparent backgrounds) and strong in-image text rendering. The generation core is a 6.15B-BF16 single-stream flow-matching DiT (30 blocks, 30 heads × 128, d 3840, SwiGLU FFN 10240; FlowMatchEuler scheduler, 1000 timesteps, shift 6.0, 12 steps at CFG 1.0, native 2048×2048). Conditioning comes from a 17B BailingMM2 multimodal LLM (BailingMoeV2ForCausalLM: 20L/2048d, 256 experts top-8 + 1 shared, first layer dense, sliding window 4096, 32768 ctx; Qwen2.5-ViT 1280d window-112 tower with full attention at blocks 7/15/23/31; Qwen2-arch 28L/1536d text-encoder connector), whose hidden states at layers 5/12/20 are projected to 2560-d caption features for the DiT. 16-channel AutoencoderKLQwenImage VAE (8.0064 scaling) supplies latents. Companion Ming-Image-0.1-Design-Layer (same DiT, multi_frame_output + learned padding) decomposes a flat design into a requested number of RGBA layers.",
"sources": [
"https://huggingface.co/inclusionAI/Ming-Image-0.1-Design",
"https://huggingface.co/inclusionAI/Ming-Image-0.1-Design/raw/main/README.md",
"https://huggingface.co/inclusionAI/Ming-Image-0.1-Design/raw/main/transformer/config.json",
"https://huggingface.co/inclusionAI/Ming-Image-0.1-Design/raw/main/mllm/config.json",
